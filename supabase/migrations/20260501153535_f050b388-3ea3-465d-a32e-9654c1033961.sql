...
BEGIN
  ...
  -- ---------- Synthetic users: shadow rows in auth.users (no password = no login) ----------
  FOR v_i IN 1..c_synth_users LOOP
    v_synth_id    := gen_random_uuid();
    v_synth_email := 'synth' || v_i || '@homolog.flow.dev';
    v_synth_name  := v_first_names[1 + (v_i % array_length(v_first_names,1))] || ' ' ||
    ...
    INSERT INTO auth.users (
      ...
      '00000000-0000-0000-0000-000000000000', v_synth_id, 'authenticated', 'authenticated', v_synth_email,
      NULL, now(),
    ...
    VALUES (v_synth_id, v_synth_name, v_synth_email, false)
