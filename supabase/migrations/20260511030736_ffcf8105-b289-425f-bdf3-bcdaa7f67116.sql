...
CREATE POLICY "Admins insert workspace integrations"
  ON public.workspace_integrations FOR INSERT TO authenticated
  WITH CHECK (public.is_workspace_admin(workspace_id, auth.uid()));

CREATE POLICY "Admins update workspace integrations"
  ON public.workspace_integrations FOR UPDATE TO authenticated
  USING (public.is_workspace_admin(workspace_id, auth.uid()));

CREATE POLICY "Admins delete workspace integrations"
...
-- Update automations trigger to support send_slack_message
      ...
      FOR action_item IN SELECT * FROM jsonb_array_elements(rule.actions) LOOP
        CASE action_item->>'type'
          ...
          WHEN 'send_slack_message' THEN
            v_channel_id := action_item->>'channel_id';
            v_message := COALESCE(action_item->>'message','');
            IF v_channel_id IS NOT NULL AND v_channel_id <> '' AND v_message <> '' THEN
              ...
              BEGIN
                ...
                applied := applied || jsonb_build_object('type','send_slack_message','channel_id',v_channel_id);
