...
BEGIN
  -- Anti-loop: trigger depth guard
  IF pg_trigger_depth() > 1 THEN
    RETURN NEW;
  END IF;

  -- Anti-loop: session flag guard (transaction-local)
  IF COALESCE(current_setting('app.automation_running', true), '') = 'true' THEN
    RETURN NEW;
  END IF;
  ...
  LOOP
    ...
    FOR cond_list IN
      SELECT v FROM (VALUES
        (rule.conditions),
        (rule.trigger_config -> 'conditions')
      ) AS t(v)
    LOOP
      IF jsonb_typeof(cond_list) = 'array' THEN
        FOR cond_item IN SELECT * FROM jsonb_array_elements(cond_list) LOOP
          field_val := NULL;

          IF (cond_item->>'field') = 'assignee'
             AND (cond_item->>'op') IN ('eq','neq')
             AND COALESCE(cond_item->>'value','') <> '' THEN
            SELECT (NEW.assignee_id::text = cond_item->>'value')
                OR EXISTS (
                  SELECT 1 FROM public.task_assignees ta
                  WHERE ta.task_id = NEW.id
                    AND ta.user_id::text = cond_item->>'value'
                )
            INTO assignee_match;
            IF (cond_item->>'op') = 'eq' THEN
              cond_ok := cond_ok AND assignee_match;
            ELSE
              cond_ok := cond_ok AND NOT assignee_match;
            END IF;
    ...
    -- Mark this transaction as currently running an automation (anti-loop).
    ...
    -- Release the flag so subsequent independent automations in the same
    -- transaction (different rules / different statements) can still run.
